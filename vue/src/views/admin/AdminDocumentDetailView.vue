...
const buildTrackerDescription = computed(() => {
  ...
  if ((hasBuildTaskSnapshot.value && hasCode(buildTaskSnapshot.value?.taskStatus, 3)) || hasCode(documentDetail.value?.indexStatus, 3)) {
    return '即使任务执行很快，这里也会保留完整阶段轨迹，方便复盘和问题定位。'
  }
...
.workspace-guidance-card-danger {
  ...
  background: rgba(185, 64, 48, 0.04);
}
...
.pipeline-headline-parent {
  border-bottom-color: rgba(67, 56, 202, 0.18);
}
...
.strategy-status-step-failed {
  border-color: rgba(185, 64, 48, 0.16);
  background: rgba(185, 64, 48, 0.04);
}

.strategy-status-step-failed .strategy-status-index {
  background: rgba(185, 64, 48, 0.14);
  color: #9f422b;
}
...
.chunk-chip-2 { background: rgba(67, 56, 202, 0.1); color: var(--color-primary-strong); }
.chunk-chip-3 { background: rgba(21, 115, 91, 0.1); color: #12644f; }
.chunk-chip-4 { background: rgba(185, 64, 48, 0.1); color: #9f422b; }
...
.inline-notice-danger {
  ...
  border: 1px solid rgba(185, 64, 48, 0.12);
  color: #9f422b;
}
...
.stage-failed { border-color: rgba(185, 64, 48, 0.14); background: rgba(255, 247, 237, 0.96); }
.stage-failed .stage-order,
.stage-failed .stage-body em { background: rgba(185, 64, 48, 0.1); color: #9f422b; }
